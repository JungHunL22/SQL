...
select a.user_id,date_trunc('month',visit_stime)::date as month,count(*) monthly_user_cnt
from ga.ga_sess a join ga.ga_users b on a.user_id=b.user_id
where b.create_time<=(date_trunc('month',visit_stime)+interval '1 month'-interval '1 days')::date -2
